...
func (s *StoreCassandra) createTables() error {
	var err error
	err = s.session.Query(`CREATE TABLE IF NOT EXISTS DesiredStates (app_guid text, app_version text, number_of_instances int, state text, package_state text, updated_at bigint, PRIMARY KEY (app_guid, app_version))`).Exec()
	if err != nil {
		return err
	}
